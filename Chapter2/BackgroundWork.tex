...
Handling program entities like database connections, file handles or even
in-memory shared state are susceptible to introduction of bugs in
...
consume which is represented as $A \rightspoon B$ and read ``consume $A$ to produce $B$''. The assumption $A$ cannot be used again after
it has been used to obtain $B$. Similarly, there are two fragments of connectives, multiplicative and additive that
arise in this logic system. Multiplicative conjunction is represented as $A \otimes B$, and additive conjunction and disjunction ($A \with B$ and $A \oplus B$).
...
The intuitionistic $A \supset B$ can be encoded in linear logic by using the modality operator as $\oc A \rightspoon B$.
...
In traditional sense of type classes in Haskell, we can think of the \Un to be a type-class with methods supporting the operation
