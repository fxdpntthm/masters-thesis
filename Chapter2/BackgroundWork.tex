...
The [LET] allows implicit parametric polymorphism. For example, the function, \HaskellF{g = \f -> (f True, f 1)} will raise a
type error as the type inference algorithm will not be able to unify the types \HaskellF{f True : Bool -> |$\alpha$|}
with \HaskellF{f 1 : Int -> |$\beta$|}. However there indeed exists a polymorphic function \HaskellF{f: |$\forall \alpha$|. |$\alpha$| -> |$\alpha$|}
that can be used to type check the function \HaskellF{g}. The \texttt{let} construct comes handy in such cases. The function \HaskellF{g} can be redefined
as \HaskellF{g = let f = \x -> x in (f True, f 1)}. The type of the function \HaskellF{f} is now computed
as \HaskellF{f: |$\forall \alpha$|. |$\alpha$| -> |$\alpha$|} and the function \HaskellF{g} is assigned a type \HaskellF{(Bool, Int)}.
    ...
    {\small
      ...
      \begin{minipage}{0.50\linewidth}
        \begin{flalign*}
          \M(\Gamma \vdash &\lambda x. M:\tau) = S_1 \circ S_2\\
            \text{where\qquad}\ S_1 &= \mathcal{U}(\tau, u_1 \rightarrow u_2)\\
      ...
      \begin{minipage}{0.45\linewidth}
        \begin{flalign*}
          ...
          \text{where\qquad}\ S_1 &= \M(\Gamma \vdash M: u \rightarrow \tau)\\
      ...
      \begin{minipage}{0.50\linewidth}
        \begin{flalign*}
          ...
          \text{where\qquad}\ S_1 &= \M(\Gamma \vdash M: u)\\
          S_2 &= \M(S_1\Gamma \cup \{x:\text{Gen}(S_1\Gamma, S_1u)\} \vdash N:\tau)\\
        \end{flalign*}
      \end{minipage}

      \fbox{Auxiliary Definition}
