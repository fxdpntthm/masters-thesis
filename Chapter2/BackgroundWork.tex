...
The [LET] allows implicit parametric polymorphism. For example, the expression, \HaskellF{g = |$\lambda$|f. (f True, f 1)} will raise a
type error as the type inference algorithm will not be able to unify the type of expression \HaskellF{f True : Bool |$\rightarrow \alpha$|}
with \HaskellF{f 1 : Int |$ \rightarrow \beta$|}. However there indeed exists a polymorphic type for \HaskellF{|$\forall \alpha. \alpha \rightarrow \alpha$|}
that can be used to type check expression \HaskellF{g}. The \texttt{let} construct comes handy in such cases. The expression \HaskellF{g} can be defined
as \HaskellF{g = let f = |$\lambda$| x. x in (f True, f 1)}. The type of the expression \HaskellF{f} is now computed
as \HaskellF{f: |$\forall \alpha$|. |$\alpha \rightarrow \alpha$|} and \HaskellF{g} is assigned a type \HaskellF{(Bool, Int)}.
