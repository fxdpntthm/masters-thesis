...
\HaskellF{f True : Bool |$\rightarrow \alpha$|} and \HaskellF{f 1 : Int |$ \rightarrow \beta$|} is not possible.
However, there indeed exists a polymorphic type \HaskellF{|$\forall \alpha. \alpha \rightarrow \alpha$|}
...
as \HaskellF{f: |$\forall \alpha$|. |$\alpha \rightarrow \alpha$|}, using [$\forall$I] rule and \HaskellF{g} is assigned a type \HaskellF{(Bool, Int)}.

\begin{figure}[h]
  \centering
  \begin{framed}
  \begin{flalign*}
    A, B, C ::= X \mid A \subset B \mid A \times B \mid A \plus B
  \end{flalign*}
\end{framed}
...
The grammar of intuitionistic logic is shown in \cref{fig:intu-logic-grammar} where $A \subset B$ denotes implication.
