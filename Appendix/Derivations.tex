...
\begin{minipage}[h]{1.0\linewidth}
  \begin{prooftree}
    ...
    \UnaryInfC{$\emptyset \mid x^{yf}:\tau \varoplus y^{xf}:\tau' \varoplus f^{xy}: \tau \rightarrow \tau' \rightarrow \upsilon \vdash f x y: \upsilon$}\RightLabel{[$\rightarrow I$]}
    \UnaryInfC{$\emptyset \mid x^{y}:\tau \varoplus y^{x}:\tau' \vdash \lambda^{\alpha}f. f x y: (\tau \rightarrow \tau' \rightarrow \upsilon) \rightarrow \upsilon$}\RightLabel{[$\rightarrow I$]}
    \UnaryInfC{$\emptyset \mid x^{\emptyset}:\tau \vdash \lambda^{\alpha}y. \lambda^{\alpha}f. f x y: \tau' \rightarrow (\tau \rightarrow \tau' \rightarrow \upsilon) \rightarrow \upsilon$}\RightLabel{[$\sepimp$ I]}
    \UnaryInfC{$\emptyset \mid I \vdash \lambda^{*}x. \lambda^{\alpha}y. \lambda^{\alpha}f. f x y: \tau \sepimp \tau' \rightarrow (\tau \rightarrow \tau' \rightarrow \upsilon) \rightarrow \upsilon$}
  \end{prooftree}
\end{minipage}\\
