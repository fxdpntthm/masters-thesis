...
does not throw exceptions. \HaskellF{openFile} throwing an exception would not be a problem as there would be
no resource leak in that case. We have two kinds of IO operations, \HaskellF{IO} that does not fail or throw exceptions and is safe,
and \HaskellF{IOF} that can fail and throw exceptions. \HaskellF{onExcept} can potentially convert a code that can fail, to a code
that does not fail by using \HaskellF{catch} function, or it may re-throw the exception after
cleaning up the resources. The sharing arrow between the two arguments of \HaskellF{onExcept} enforces
