...
cause problems at runtime. To solve this problem, we introduce the % concept of unrestricted type similar to Quill, and
...
exceptions where the part of code that is responsible to clean up resources. In Haskell, error handing is done using \HaskellF{MonadError}.
...
and \HaskellF{IOF} that can fail and throw exceptions. \HaskellF{throw} function can be used by a function to start
the processing of the exception. \HaskellF{catch} function can potentially convert a code that can fail, to a code
that does not fail. The \HaskellF{onExcept} function gives a way to clean up resources after the code throws an exception. It may re-throw the
exception after cleaning up the resources or a \HaskellF{catch} function can be used to handle the failure case.
The sharing arrow between the two arguments of \HaskellF{onExcept} enables the clean up block of code to be executed on the same resources
