...
The tautology rule ([ID]) is a simple context lookup for checking the type of the term.
The exchange rule ([EXCH]) justifies that the order in which the typing judgements are witnessed
in the multiset is insignificant and we can commute the individual term typing schemes within the multiset context
with no effect to the typing derivations. The contraction sharing rule [CTR-SH] and weakening sharing rule [WKN-SH] say that we can duplicate
or drop judgements from the typing contexts as per our will as we know they are in sharing with other
