...
% TODO!! add the tree graph here

\begin{figure}[h]
  \begin{framed}
    \begin{flalign*}
      \text{Term Variables}\ \ \      x, y, z         &\in \text{Var} \nonumber\\
      \text{Patterns}\ \ \            p               &::= x \mid C \overset{\rightarrow}{x}\nonumber\\
      \text{Expressions}\ \ \         M, N            &::= x \mid \lambda^{*}x. M \mid \lambda^{\alpha}x. M \mid M N\nonumber\\
                                                      &\mid \Let{x}{M}{N} \mid \texttt{case}\ M\ \texttt{of}\ \{p_i \mapsto N_i\}_i\nonumber
    \end{flalign*}
  \end{framed}
...
\begin{figure}[h]
  \begin{framed}
    ...
    \begin{minipage}{1\textwidth}
      \begin{prooftree}
        \AxiomC{$P \mid \Gamma \bullet \Delta \vdash M: \tau_1 \oplus \tau_2 \oplus \cdots \oplus \tau_n$}
        \AxiomC{$P \mid \Delta \vdash p_i :\tau_i$}
        \AxiomC{$P \mid \Gamma \bullet \Delta \vdash N_i:v$}\RightLabel{$[C_i I]$}
        \TrinaryInfC{$P \mid \Gamma \bullet \Delta \vdash \texttt{case}\ M\ \texttt{of}\ \{p_i \mapsto N_i\}_i : v $}
      \end{prooftree}
    \end{minipage}
    % sharing constructor
    \begin{minipage}{1\textwidth}
      \begin{prooftree}
        ...
        \AxiomC{$P \mid \Delta \equiv \Delta'(x_1 : \tau_1 ; x_2 : \tau_2 ; \ldots ; x_n:\tau_n)$}\RightLabel{$[\text{C-sh}]$}
        \BinaryInfC{$P \mid \Gamma; \Delta \vdash C \overset{\rightarrow}{x} : T $}
      \end{prooftree}
    \end{minipage}
    ...
    \begin{minipage}{1\textwidth}
      \begin{prooftree}
        ...
        \AxiomC{$P \mid \Delta \equiv \Delta'(x_1: \tau_1, x_2:\tau_2, \dots ,x_n:\tau_n)$}\RightLabel{$[\text{C-se}]$}
        \BinaryInfC{$P \mid \Gamma, \Delta \vdash C \overset{\rightarrow}{x} : T $}
      \end{prooftree}
    \end{minipage}
...
\begin{figure}[h]
  \begin{framed}
    ...
    \begin{minipage}{1\linewidth}
      \begin{flalign*}
        \M(S, X;\Gamma \vdash x : \tau) &= ([\overset{\rightarrow}{u} / \overset{\rightarrow}{t}]P), S' \circ S, \{x\} \nonumber \\
        \text{where}\ (x : \forall \overset{\rightarrow}{t}. P \Rightarrow \nu) &\in S \Gamma \nonumber\\
        S' &= \Unf([\overset{\rightarrow}{u} / \overset{\rightarrow}{t}]\nu, S \tau)
      \end{flalign*}
    \end{minipage}
    ...
    \begin{minipage}{1\linewidth}
      \begin{flalign*}
        ...
        \sigma &= \text{GenI}(R\Gamma; R(P \Rightarrow u_1)) \nonumber\\
