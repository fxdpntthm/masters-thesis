...
record types and sub-typing \citep{jones_theory_1994}.

The type language is modified from \cref{fig:hm-lang} to incorporate
qualified types shown in \cref{fig:qualifed-types}. The changes to the \textbf{HM} type system are \colorbox{blue!30}{highlighted}.
$P$ and $Q$ range over finite set of predicates and $\pi$ and $\omega$ range over predicates. The typing rules are also modified
from \cref{fig:hm-system} to add 2 new rules [$=>$I] and [$=>$E] for introduction and elimination of qualified types respectively
as shown in \cref{fig:qualified-types-rules}.
...
\begin{figure}[h]
  ...
  \begin{flalign*}
    ...
    \text{Type Scheme}\ \ \ \sigma      &::= \tau \mid \forall \vec{t}. \rho\\
           \text{Expressions}\ \ \ M, N &::= x: \sigma \mid \lambda x. M \mid M N \mid \Let{x}{M}{N}
  \end{flalign*}
\end{framed}
