...
$\Gamma$ denotes the collection of three-tuple containg the variable, its type and its sharing information.
...
\begin{figure}[h]
  \begin{framed}
    ...
    \begin{minipage}{.5\textwidth}
      \begin{prooftree}
        \AxiomC{$\Gamma \sqcup \Delta \vdash M : \sigma$} \RightLabel{[EXCH]}
        \UnaryInfC{$P \mid \Delta \sqcup \Gamma \vdash M : \sigma $}
      \end{prooftree}
    \end{minipage}
        % CTR UN
    \begin{minipage}{.5\textwidth}
...
We collapse the application rules
...
not be in scope other than its use in $N$. We divide the typing context into multiple parts.
...
{\color{red} The conditions outlined do not clearly look as if they are syntax directed and in the cases where the type checker cannot directly infer if the
