...
of resources of the fully realized type. {\color{blue} is this justifiable?.}
...
\begin{figure}
  \begin{framed}
    ...
    \begin{minipage}{1.0\textwidth}
      \begin{prooftree}
        \AxiomC{$P \vdash \Gamma\ \texttt{un}$}
        \AxiomC{$(P => \tau) \sqsubseteq \sigma$} \RightLabel{[VAR$^s$]}
        \BinaryInfC{$P \mid \Gamma, x^{\bar{y}} : \sigma \vdashs x : \tau $}
      \end{prooftree}
    \end{minipage}
    % Let^s
    \begin{minipage}{1.0\textwidth}
      \begin{prooftree}
        \AxiomC{$Q \mid \Gamma_x',\Delta \vdashs M: v$}
        \AxiomC{$\sigma = \texttt{Gen}(\{\Gamma' \cup \Delta\}, Q => v)$}
        \AxiomC{$P \mid \Gamma_x,x:\sigma, \Delta \vdashs N:\tau$}
        \AxiomC{$P \vdash \Delta\ \texttt{un}$}\RightLabel{[Let$^s$]}
        \QuaternaryInfC{$P \mid \Gamma,\Gamma',\Delta \vdashs \Let{x}{M}{N} : \tau $}
      \end{prooftree}
    \end{minipage}
    % -*>I^s
    \begin{minipage}{1.0\textwidth}
      \begin{prooftree}
        \AxiomC{$P \mid \Gamma, x^{\emptyset}:\tau \vdashs M: v$}
        \AxiomC{$P => \SeFun{\phi}$}
    ...
    \begin{minipage}{1.0\textwidth}
      \begin{prooftree}
        \AxiomC{$P \mid \Gamma^{[\emptyset \mapsto \{x\}]}, x^{\texttt{Vars}(\Gamma)}:\tau \vdashs M: v$}
        \AxiomC{$P => \ShFun{\phi}$}
    ...
    \begin{minipage}{1.0\textwidth}
      \begin{prooftree}
        ...
        \AxiomC{$P \mid \Gamma,\Delta \vdashs M: \phi v \tau\ \ \ \ \
          P \mid  \Gamma',\Delta \vdashs N: v$}\noLine
        \BinaryInfC{if \texttt{Used}($\Gamma$) = \texttt{Used}($\Gamma'$) then ($P => \ShFun{\phi}$) else ($P => \SeFun{\phi}$)}\RightLabel{[App$^s$]}
        \UnaryInfC{$P \mid \Gamma,\Gamma',\Delta \vdashs M N: \tau$}
      \end{prooftree}
    \end{minipage}
    % sum types
    \begin{minipage}{0.5\textwidth}
    ...
    \begin{minipage}{1.0\textwidth}
      \begin{prooftree}
        \AxiomC{$P \vdash  \Delta\ \texttt{un}$}
        \AxiomC{$P \mid  \Gamma,\Delta \vdashs M: \tau \oplus \tau'$}
        \AxiomC{$P \mid  \Gamma'_x,\Delta,x:\tau \vdashs N: v$}
        \AxiomC{$P \mid  \Gamma'_y,\Delta, y:\tau' \vdashs N': v$}\RightLabel{[$\oplus$E$^s$]}
        \QuaternaryInfC{$P \mid \Gamma,\Gamma', \Delta \vdashs \Case{M}{\{\texttt{inl}\ x \mapsto N; \texttt{inr}\ y \mapsto N'\}}:v$}
      \end{prooftree}
      \end{minipage}
    \end{framed}
