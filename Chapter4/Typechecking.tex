...
of returning the type of the variable it returns the sharing information. Domain of $\Psi$ defined as
...
of the constructor types $C$ and $C'$.{\color{red} Some more about robinsons algorithm here}.

We define some auxilary functions in \cref{fig:aux-defs}. $\texttt{Weaken}(x, \sigma, \Sigma)$ adds the unrestricted
predicate to the type $\sigma$ if it does not belong to $\Sigma$. $\texttt{Un}(\Gamma)$ adds an unrestricted
