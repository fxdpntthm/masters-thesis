...
the binders would introduce fresh names. The reason to use multisets can now be explained
  ...
  \begin{framed}
    ...
    \begin{minipage}{1.0\textwidth}
      \begin{prooftree}
        ...
        \BinaryInfC{$((\texttt{Used}(\Gamma) = \texttt{Used}(\Gamma')) \wedge (P => \ShFun{\phi}))$}\noLine
        \UnaryInfC{$ \vee ((\texttt{Vars}(\Gamma) \# \texttt{Used}(\Gamma') \wedge \texttt{Vars}(\Gamma') \# \texttt{Used}(\Gamma)) \wedge (P => \SeFun{\phi})))$}\RightLabel{[App$^s$]}
...
not be in scope out of $N$. We divide the typing context into multiple parts. $\Gamma$ contains the variables that exists exclusively in $M$ and $\Gamma'$ which
...
\TODO{We will also prove equivalence between the typing rules and syntax directed typing rules of our system?}
