...
how we can encode sums in \cref{sec:sums} and multiplicative and additive products in \cref{sec:products}.
...
The modified type system is shown in \cref{fig:qub-kind-extension}. all types have kind $\star$ and the
kind of the type constructors depends on its arity. $\sepimp$, $\overset{!}{\sepimp}$, $\rightarrow$ and $\overset{!}{\rightarrow}$
...
Int, Float will have a kind $\star$. The language of type constructors is extendible
...
The kind system changes the treatement to the type inference algorithm in some detail.
All types and type constructors have to be annotated with their kind. The language
of constructors is a system of combinators without reduction rules. The type constructor
application rule that computes kinds is given in \cref{fig:constructor-application} where
$T$ is of kind $\kappa' \rightarrow \kappa$ and $T'$ is of kind $\kappa'$. The application
